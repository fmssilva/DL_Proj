...
class ResidualCNN(nn.Module):
    def __init__(self, in_channels: int = 3, dropout: float = 0.4):
        ...
        self.skip1 = nn.Sequential(nn.Conv2d(in_channels, 64, 1), nn.BatchNorm2d(64))
        ...
        self.skip2 = nn.Sequential(nn.Conv2d(64, 128, 1), nn.BatchNorm2d(128))
        ...
        self.skip3 = nn.Sequential(nn.Conv2d(128, 256, 1), nn.BatchNorm2d(256))
    ...
    def forward(self, x):
        x = self.pool(self.relu(self.block1(x) + self.skip1(x)))
        x = self.pool(self.relu(self.block2(x) + self.skip2(x)))
        x = self.pool(self.relu(self.block3(x) + self.skip3(x)))
        x = self.classifier(x)
        return x